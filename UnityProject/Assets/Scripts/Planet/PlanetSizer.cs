...
namespace HotJupiter
{
	public class PlanetSizer : MonoBehaviour
	{
		...
		private void SetPlanetSize()
		{
			...
			{
				sphereHexGrids[gridIndex].transform.localScale =
					Vector3.one
					* (
						planetRadius
						+ HexMapHelper.gridFirstAltitudeOffset
						+ ((gridIndex - 1) * HexMapHelper.gridAltitudeOffsets)
					)
					* spherePrimitiveRadiusMultiplier;
			}

			for (int tileIndex = 0; tileIndex < tileCount; tileIndex++)
			{
				basePlanetSphere.SetTileExtrudeAmount(
					tileIndex,
					Random.Range(0, sphereHexGrids.Count) * (sphereHexGrids.Count - 1)
				);
			}
