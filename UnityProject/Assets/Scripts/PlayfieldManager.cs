...
public class PlayfieldManager : MonoBehaviour
{
    ...
    public struct SimulationTimePeriod{
        ...
        float duration { get {return end - start;} }

        public SimulationTimePeriod(float start, float end){
            this.start = start;
            if(end < start) throw new System.ArgumentException("Provided end time must be greater than start time.");
            this.end = end;
        }
        ...
        {
            return start.GetHashCode().WrapShift(2) 
                ^ end.GetHashCode();
        }
    }
    ...
    private Dictionary<Tile, HashSet<TileReservation>> tileReservations = new Dictionary<Tile, HashSet<TileReservation>>();
    private Dictionary<IHaveTileFootprint, HashSet<HashSet<TileReservation>>> reservationHolderMap = new Dictionary<IHaveTileFootprint, HashSet<HashSet<TileReservation>>>();
    ...
    public static List<IHaveTileFootprint> GetTileOccupantsAtTime(Tile tile, float simulationTime){
        HashSet<TileReservation> tileReservations;
    ...
    /*public static bool TryReserveTile(FootprintTile tile, IHaveTileFootprint claimant, SimulationTimePeriod timePeriod){
        ...
        if(instance.tileReservations.TryGetValue(tile.tile, out tileReservations)){
            ...
            if(tile.obstacleType != TileObstacleType.Empty){
                foreach(TileReservation currentReservation in tileReservations){
                    if(currentReservation.reservationPeriod.HasOverlap(timePeriod)){ //There is an overlapping reservation
                        if(currentReservation.obstacleType == TileObstacleType.Solid){ //No passing through solids
                            return false;
                        }else if(currentReservation.obstacleType == TileObstacleType.Semisolid
                            && timePeriod.WithinPeriod(TimeManager.TurnDuration)){ //No stopping at semi-solids at end of turn
                            return false;
                        }
                    }
                }
        ...
        tileReservations.Add(new TileReservation() {reservationHolder = claimant, obstacleType = tile.obstacleType, reservationPeriod = timePeriod});
    ...
    public static void RemoveTileReservations(Tile tile, IHaveTileFootprint reservationHolder){
        ...
        if(instance.tileReservations.TryGetValue(tile, out currentTileReservations)){
            int result = currentTileReservations.RemoveWhere((x) => x.reservationHolder == reservationHolder);
            if(result == 0) Debug.LogWarning($"Reservation Holder {reservationHolder} has no reservations on tile {tile}.");

            HashSet<HashSet<TileReservation>> reservationSets;
            if(instance.reservationHolderMap.TryGetValue(reservationHolder, out reservationSets)){
                bool setResult = reservationSets.Remove(currentTileReservations);
                if(setResult == false) Debug.LogWarning($"Reservation Holder {reservationHolder} is missing reservation on tile {tile}.");
                if(reservationSets.Count == 0){
                    instance.reservationHolderMap.Remove(reservationHolder);
                }
            }else{
                Debug.LogWarning($"Reservation Holder {reservationHolder} has no reservations.");
            }
        }else{
    ...
    public static void RemoveAllReservations(IHaveTileFootprint reservationHolder){
        ...
        if(instance.reservationHolderMap.TryGetValue(reservationHolder, out tileReservationSets)){
            foreach(HashSet<TileReservation> set in tileReservationSets){
                ...
                if(result == 0) Debug.LogWarning($"Reservation Holder {reservationHolder} is missing a reservation.");
            }
