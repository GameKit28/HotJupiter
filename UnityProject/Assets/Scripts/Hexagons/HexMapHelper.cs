...
namespace HotJupiter
{
	...
	public struct TileWithFacing : ITile
	{
		...
		public TileWithFacing(
			TileCoords position,
			TileCoords facing,
			TileLevel level = new TileLevel()
		)
		{
		...
		public override bool Equals(object obj)
		{
			...
			return this._level == otherStruct._level
				&& this._position == otherStruct._position
				&& this.facing == otherStruct.facing;
		}

		public override int GetHashCode()
		{
			return facing.GetHashCode().WrapShift(4)
				^ _position.GetHashCode().WrapShift(2)
				^ _level.GetHashCode();
		}
	...
	public class HexMapHelper : MonoBehaviour
	{
		...
		{
			return instance.baseHexasphere.tiles[tile.index].neighbours.Length == 6
				? TileShape.Hexagon
				: TileShape.Pentagon;
		}

		public static TileCoords GetTileInHexDirection(
			TileCoords startTile,
			TileCoords startFacing,
		...
		)
		{
			...
			if (tile.neighbours.Length == 6)
			{
				...
				for (int neighborIndex = 0; neighborIndex < tile.neighbours.Length; neighborIndex++)
				{
					...
					if (neighborTile.index == startFacing.index)
					{
						...
						{
							index = tile.neighbours[(neighborIndex + (int)direction) % 6].index
						};
					}
				}
			...
			{
				throw new Exception(
					"The startTile is not a Hexagon. Use GetTileInPentaDirection instead."
				);
			}
		}

		public static TileCoords GetTileInPentaDirection(
			TileCoords startTile,
			TileCoords startFacing,
			PentaDirection direction
		)
		{
			HexasphereGrid.Tile tile = instance.baseHexasphere.tiles[startTile.index];
			...
			{
				throw new Exception(
					"The startTile is not a Pentagon. Use GetTileInHexDirection instead."
				);
			}
		}
		...
		public static Vector3 GetWorldPointFromTile(
		...
		)
		{
			...
			{
				return instance.baseHexasphere.GetTileCenter(tileCoords.index)
					+ (GetTileNormal(tileCoords) * GetAltitudeFromLevel(level));
			}
		}
		...
		public static Quaternion GetRotationFromFacing(
			TileCoords startingTile,
			TileCoords tileFacing
		)
		{
			return Quaternion.LookRotation(
				GetFacingVector(startingTile, tileFacing),
				GetTileNormal(startingTile)
			);
		}
		...
		{
			return (
				GetWorldPointFromTile(tileFacing) - GetWorldPointFromTile(startTile)
			).normalized;
		}
		...
		public static List<Vector3> NeighborVectors(TileCoords startTile)
		{
			...
			{
				neighborVectors.Add(
					(instance.baseHexasphere.GetTileCenter(tile.index) - startPos).normalized
				);
			}
		...
		{
			return Mathf.Clamp(
				Mathf.RoundToInt((altitude + gridFirstAltitudeOffset) / gridAltitudeOffsets),
				TileLevel.MIN,
				TileLevel.MAX
			);
		}
