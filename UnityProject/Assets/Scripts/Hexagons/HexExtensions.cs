...
namespace HotJupiter
{
	public static class HexExtensions
	{
		...
		private static TileCoords GetFacingWhenEnteringPentagon(
			TileCoords currentHex,
			TileCoords newPenta,
			PentagonTraversalStrategy strategy
		)
		{
			if (strategy == PentagonTraversalStrategy.LeanLeft)
			{
				return HexMapHelper.GetTileInPentaDirection(
					newPenta,
					currentHex,
					PentaDirection.BackwardRight
				);
			}
			else
			{
		...
		private static TileCoords GetNewTileWhenLeavingPentagon(
			TileCoords currentPenta,
			TileCoords currentFacing,
		...
		public static TileWithFacing TraversePlanar(
			this TileWithFacing startVec,
			HexDirection direction,
			int steps = 1,
			PentagonTraversalStrategy strategy = PentagonTraversalStrategy.LeanLeft
		)
		{
		...
		public static TilePath TraversePlanar(
			this TilePath activePath,
			HexDirection direction,
			int steps = 1,
			PentagonTraversalStrategy strategy = PentagonTraversalStrategy.LeanLeft
		)
		{
			...
			for (int step = 0; step < steps; step++)
			{
				...
				{
					newTileCoords = HexMapHelper.GetTileInHexDirection(
				...
				else
				{
					...
					newTileCoords = GetNewTileWhenLeavingPentagon(
						currentTile.position,
						currentTile.facing,
						direction,
						strategy
				...
				{
					currentTile.facing = HexMapHelper.GetTileInHexDirection(
						newTileCoords,
						currentTile.position,
						HexDirection.Backward
					);
				}
				else
				{
					//We're stepping into a Pentagon with Hex Directions. What do we do?
					currentTile.facing = GetFacingWhenEnteringPentagon(
						currentTile.position,
						newTileCoords,
						strategy
					);
				...
				currentTile = new TileWithFacing(
					newTileCoords,
					currentTile.facing,
					currentTile.level
				);
		...
		public static TileWithFacing TraverseVertical(
			this TileWithFacing startVec,
			int relativeAscention
		)
		{
			TilePath path = new TilePath(startVec);
			...
			for (int step = 0; step < Mathf.Abs(relativeAscention); step++)
			{
				...
				else
				{
					throw new System.Exception(
						$"Attempted to traverse to a TileLevel outside of bounds. {currentTile} Attempted Level: {currentTile.level + Mathf.Clamp(relativeAscention, -1, 1)}"
					);
				}
				currentTile = newTile;
				activePath.AppendTile(currentTile);
		...
		public static TilePath Face(this TilePath activePath, HexDirection direction)
		{
			...
			if (HexMapHelper.GetTileShape(currentTile.position) == TileShape.Hexagon)
			{
				newFacing = HexMapHelper.GetTileInHexDirection(
					currentTile.position,
					currentTile.facing,
					direction
				);
			...
			{
				newFacing = GetNewTileWhenLeavingPentagon(
					currentTile.position,
					currentTile.facing,
					direction,
					PentagonTraversalStrategy.LeanLeft
				);
			}
			activePath.AppendTile(
				new TileWithFacing(currentTile.position, newFacing, currentTile.level)
			);
			return activePath;
		}

		public static HexDirection RotateClockwise(
			this HexDirection startingDirection,
			int steps = 1
		)
		{
		...
		public static HexDirection RotateCounterClockwise(
			this HexDirection startingDirection,
			int steps = 1
		)
		{
