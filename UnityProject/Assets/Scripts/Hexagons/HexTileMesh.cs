			...
			for (int footPartIndex = 0; footPartIndex < tiles.Count; footPartIndex++)
			{
				...
				if (footTile != null)
				{
					...
					vertices[0] = parent.InverseTransformPoint(
						footTile.center * 2f * HexMapHelper.GetRadialOffsetFromLevel(footPart.level)
					);
					for (int i = 0; i < footTile.vertices.Length; i++)
					{
						vertices[i + 1] = parent.InverseTransformPoint(
							footTile.vertices[i]
								* 2f
								* HexMapHelper.GetRadialOffsetFromLevel(footPart.level)
						);
					}

					polygons[footPartIndex].SetVertices(vertices);
					polygons[footPartIndex]
						.SetTriangles(
