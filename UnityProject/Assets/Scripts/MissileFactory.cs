...
namespace HotJupiter
{
	public class MissileFactory : MonoBehaviour
	{
		...
		public static void SpawnMissile(
			TileWithFacing originTile,
			BaseGamePiece spawningPiece,
			MissileStats template
		)
		{
			...
			newMissile.GetComponentInChildren<PieceController>().worldBase.transform.position =
				HexMapHelper.GetWorldPointFromTile(originTile.position, originTile.level);
			newMissile.GetComponentInChildren<PieceController>().worldModel.transform.rotation =
				HexMapHelper.GetRotationFromFacing(originTile.position, originTile.facing);
			...
			newMissile
				.GetComponentInChildren<PieceController>()
