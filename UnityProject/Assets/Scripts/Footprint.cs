...
namespace HotJupiter
{
	...
	public class StaticFootprint : FootprintBase
	{
		...
		public StaticFootprint(
			IHaveTileFootprint owner,
			RelativeFootprintTemplate footprint,
			TileWithFacing pivotVec
		)
			: base(owner)
		{
	...
	public struct FootprintTile : ITile
	{
		...
		{
			get { return tile.level; }
			set { tile.level = value; }
		}
	...
	public abstract class FootprintBase
	{
		...
		protected List<FootprintTile> CalculateTilesFromTemplate(
			RelativeFootprintTemplate footprint,
			TileWithFacing pivotVec
		)
		{
			...
			foreach (var part in footprint.footprintParts)
			{
				...
				{
					newVec = newVec.TraversePlanar(
						part.relativePosStep1.direction,
						part.relativePosStep1.step
					);
				}
				if (part.relativePosStep2.step > 0)
				{
					newVec = newVec.TraversePlanar(
						part.relativePosStep2.direction,
						part.relativePosStep2.step
					);
				}
				newVec.TraverseVertical(part.relativeLevel);
				footprintList.Add(
					new FootprintTile(newVec.position, newVec.level, part.obstacleType)
				);
			}
