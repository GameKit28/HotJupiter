...
namespace HotJupiter
{
	public partial class CommandPointViewFsm
	{
		public class SelectedState : MeFsmState<CommandPointViewFsm>
		{
			...
			{
				ParentFsm.spriteRenderer.color = HexMapUI.GetLevelColor(
					ParentFsm.model.destinationTile.level
				);
			}
			...
			void Update()
			{
				...
				if (swapCountdown < 0)
				{
					ParentFsm.spriteRenderer.color = isBaseColor
						? Color.cyan
						: HexMapUI.GetLevelColor(ParentFsm.model.destinationTile.level);
