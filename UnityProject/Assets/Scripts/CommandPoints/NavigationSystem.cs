...
namespace HotJupiter
{
	public class NavigationSystem : MonoBehaviour
	{
		...
		{
			public struct NewPointSelected : IEvent
			{
				public CommandPointController SelectedPoint;
			}
		}
		...
		private List<CommandPointController> availableCommandPoints =
			new List<CommandPointController>();
		...
		public void GenerateCommandPoints()
		{
			...
			if (pieceController.gamePiece.currentVelocity == pieceController.pieceTemplate.TopSpeed)
				gForceModifier = 1;
			...
			{
				position = pieceController.GetPivotTilePosition(),
				facing = pieceController.GetTileFacing(),
				level = pieceController.GetPivotTileLevel()
			};

			var defalutSelectedPoint = InstantiateCommandPoint(
				new TilePath(startingVec).TraversePlanar(
			...
			if (
				pieceController.gamePiece.currentVelocity < pieceController.pieceTemplate.TopSpeed
				&& pieceController.pieceTemplate.canAccelerate
			)
			{
			...
			if (
				pieceController.gamePiece.currentVelocity > 2
				&& pieceController.pieceTemplate.canDecelerate
			)
			{
				InstantiateCommandPoint(
					new TilePath(startingVec).TraversePlanar(
			...
			InstantiateCommandPoint(
				new TilePath(startingVec)
					.TraversePlanar(HexDirection.Forward, pieceController.gamePiece.currentVelocity)
					.Face(HexDirection.ForwardLeft),
				pieceController.gamePiece.currentVelocity,
				0 + gForceModifier
			);
			...
			InstantiateCommandPoint(
				new TilePath(startingVec)
					.TraversePlanar(HexDirection.Forward, pieceController.gamePiece.currentVelocity)
					.Face(HexDirection.ForwardRight),
				pieceController.gamePiece.currentVelocity,
				0 + gForceModifier
			);
					...
					if (pieceController.pieceTemplate.canStrafe)
					{
						...
						InstantiateCommandPoint(
							new TilePath(startingVec)
								.TraversePlanar(
									HexDirection.Forward,
									pieceController.gamePiece.currentVelocity - manu
								)
								.TraversePlanar(HexDirection.ForwardLeft, manu)
								.Face(HexDirection.ForwardRight),
							pieceController.gamePiece.currentVelocity,
						...
						InstantiateCommandPoint(
							new TilePath(startingVec)
								...
								.TraversePlanar(HexDirection.ForwardRight, manu)
								.Face(HexDirection.ForwardLeft),
							pieceController.gamePiece.currentVelocity,
							baseGForce + gForceModifier
						);
			...
			{
				if (
					pieceController.GetPivotTileLevel() < TileLevel.MAX
					&& pieceController.gamePiece.currentVelocity >= 1
				)
				{
					InstantiateCommandPoint(
						new TilePath(startingVec)
							...
							.TraverseVertical(1)
							.TraversePlanar(HexDirection.Forward, 1),
						pieceController.gamePiece.currentVelocity,
						1 + gForceModifier
					);
				}
			...
			eventPublisher.Publish(
				new Events.NewPointSelected() { SelectedPoint = defalutSelectedPoint }
			);
		...
		private CommandPointController InstantiateCommandPoint(
			TilePath path,
			int endVelocity,
			int gForce
		)
		{
			CommandPointController commandPoint = GameObject
				.Instantiate(commandPointPrefab, transform.position, transform.rotation, transform)
				.GetComponent<CommandPointController>();

			commandPoint.SetSource(
				pieceController.worldModel.transform.position,
				HexMapHelper.GetFacingVector(
					pieceController.gamePiece.currentTile.position,
					pieceController.gamePiece.currentTile.facing
				)
			);
		...
		private void OnCommandPointClicked(CommandPointViewFsm.Events.CommandPointClicked @event)
		{
			...
			eventPublisher.Publish(
				new Events.NewPointSelected() { SelectedPoint = @event.CommandPoint }
			);
		}
	}
}
