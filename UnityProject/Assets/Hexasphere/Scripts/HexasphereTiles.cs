...
namespace HexasphereGrid {
	...
	public partial class Hexasphere: MonoBehaviour {
		...
		public bool SetTileMaterial (int tileIndex, Material mat, bool temporary = false) {
			...
			if (temporary) {
				...
				if (mat.HasProperty("_Color")) {
					...
					matColor = mat.GetColor("_BaseColor");
                }
				...
				if (mat.HasProperty("_MainTex")) {
					...
					matTexture = mat.GetTexture("_BaseMap");
                }
				if (matTexture != null) {
			...
			if (highlightMaterial != null && tile == lastHighlightedTile) {
				...
				if (srcMat != null) {
					...
					if (srcMat.HasProperty("_Color")) {
						color = srcMat.color;
                    } else if (srcMat.HasProperty("_BaseColor")) {
						color = srcMat.GetColor("_BaseColor");
                    }
					highlightMaterial.SetColor ("_Color2", color);
					...
					if (srcMat.HasProperty("_MainTex")) {
						...
						tempMatTexture = srcMat.GetTexture("_BaseMap");
					}
					if (tempMatTexture != null && highlightMaterial.HasProperty ("_MainTex")) {
		...
		public float GetTileVertex0Angle (int tileIndex) {
			...
			Vector3 tileCenter = (tiles [tileIndex].vertices [0] + tiles [tileIndex].vertices [3]) * 0.5f; // tiles [tileIndex].center;
		...
		public void ClearTile (int tileIndex, bool clearTemporaryColor = false, bool clearAllColors = true, bool clearObstacles = true) {
			...
			if (clearAllColors) {
				if (tile.customMat != null) {
					if (tile.customMat.HasProperty ("_MainTex") && tile.customMat.mainTexture != null) {
						pendingTextureArrayUpdate = true;
					}
					tile.customMat = null;
				}
